        ...
        button.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                if( TextUtils.isEmpty(expressionText.getText()) && TextUtils.isEmpty(derivativeText.getText())){
                    expressionText.setError( "Expression is mandatory" );
                    derivativeText.setError( "Derivative is mandatory" );
                    return;
                }
                ...
                EditText episolonText=(EditText)findViewById(R.id.EPSILONText);
                String epsilonString=episolonText.getText().toString();
                if(null!=epsilonString && !"".equals(epsilonString)){
                     epsilon=Double.parseDouble(epsilonString);
                }

                if(epsilon>=0.1){
                    episolonText.setError( "Epsilon cannot be greater then 0.1" );
                    return;
                }
                ...
                LinkedHashMap<Integer,Double> solution=equationSolver.solveNewtonRaphson(expression,derivative,iterations,epsilon,initialGuess);
